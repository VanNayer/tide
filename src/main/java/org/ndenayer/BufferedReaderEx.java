 ...
 *
 * BufferedReaderEx is a modify version of the original BufferedReader
 * with the add of the skipOnLine() method.
 * 
 * @author Per Bothner (bothner@cygnus.com)
 * @author Aaron M. Renn (arenn@urbanophile.com)
 * @author Regis Cosnier
 */
public class BufferedReaderEx extends Reader
{
	...
	static final int DEFAULT_BUFFER_SIZE = 2048; // 4096 max for the Nokia //8192;
	/**
	...
	/**
	 * Create a new <code>BufferedReaderEx</code> that will read from the
	 ...
	 */
	public BufferedReaderEx(Reader in, int size)
	{
		//super(in.lock);
		super(in);
		if (size <= 0)
		...
		synchronized (lock)
		{
			...
			if (count > avail)
			{
				...
				else // pos >= limit
				{
					...
					{
						markPos = -1;
					} // read too far - invalidate the mark.
					if (pos > limit)
					{
	...
	public String readLine() throws IOException
	{
		...
		if (sbuf == null)
		{
                        // Nokia does not support initial StringBuffer capacity
	 ...
	 * @exception IOException If an error occurs
	 */
	public void skipOneLine() throws IOException
	{
		checkStatus();
		// Handle the special case where a previous readLine (with no intervening
		// reads/skips) had a '\r' at the end of the buffer.
